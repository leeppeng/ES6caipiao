...
var syy = new _lottery2.default(); //兼容es7
//import './class/lesson';
...
var Base = function () {
  ...
  */
		...
		value: function getCount() {
			var self = this;
			var active = (0, _jquery2.default)('.boll-list .btn-boll-active').length; //选中号码的长度
			var count = self.computeCount(active, self.cur_play); //计算注数
			var range = self.computeBonus(active, self.cur_play); //计算奖金范围（computeBonus返回两个数）
			var money = count * 2; //要花的钱数
			var win1 = range[0] - money; //最小盈利额
			var win2 = range[1] - money; //最大盈利额
			var tpl = void 0; //
			var c1 = win1 < 0 && win2 < 0 ? Math.abs(win1) : win1; //亏损状态
			var c2 = win1 < 0 && win2 < 0 ? Math.abs(win2) : win2; //亏损状态
			if (count === 0) {
				//如果注数为0
...
var Timer = function () {
	...
	_createClass(Timer, [{
		...
		value: function countdown(end, update, handle) {
			//end:结束时间；update:更新时间的回调；handle:倒计时结束后的回调函数
			var now = new Date().getTime(); //获取当前时间
			var self = this; //用self获取当前指针
			if (now - end > 0) {
				handle.call(self); //当前时间-结束时间>0；倒计时结束
			} else {
				var last_time = end - now;
				var px_d = 1000 * 60 * 60 * 24; //一天是多少毫秒
				var px_h = 1000 * 60 * 60; //一小时是多少毫秒
				var px_m = 1000 * 60; //一分是多少毫秒
				var px_s = 1000; //一秒是多少毫秒
				...
				if (r.length || h > 0) {
					//r.length判断长度：1、如果天是0，前面就不能出现天。2、还要判断有没有天这种情况
					r.push('<em>' + h + '</em>\u65F6');
				}
				if (r.length || m > 0) {
					r.push('<em>' + m + '</em>\u65F6');
...
var Calculate = function () {
  ...
  */
		value: function computeCount(active, play_name) {
			var count = 0; //注数
			var exist = this.play_list.has(play_name); //判断玩法是否存在
			var arr = new Array(active).fill('0'); //新建一个长度为active，每个值为0的数组
			if (exist && play_name.at(0) === 'r') {
				//play_name.split('')[1]:R2/R3/R4...中的数
  ...
  */
		...
		/*
  [combine 组合运算] 静态方法用static
  @param  {array}  arr  [参数 组合运算的数组]
